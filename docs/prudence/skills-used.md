...
| Hosted role-model verification (2026-09-05) | `security` (Postgres roles), `supabase` platform docs | Verified hosted `postgres` is non-superuser (roles-superuser docs) → `ALTER ... OWNER TO dealsure_owner` requires the migration-created role membership; restored `grant dealsure_owner to postgres` in 0001 as a **hosted requirement** (the earlier "Docker-only hack" framing was wrong) and corrected the ownership decision doc + test-suite fixture that assumed otherwise | `supabase/migrations/0001`, `docs/architecture/database-ownership-decision.md`, `supabase/tests/rls_authorization.sql` |
